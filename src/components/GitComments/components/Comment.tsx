...
import { getTimeToNow, getTotalTime } from "@utils/application/time";
...
const CommentSay = ({comment}: CommentProps) => {
    ...
    return <div class="grow border-2 border-slate-100 p-2 ml-4 rounded-lg">
        <div class="pb-4">
            <div>
                <a href={comment.user?.html_url || ''} target='__blank'>{comment.user?.login || 'anoymous'}</a> 发表于 {getTotalTime(comment.updated_at)} {getTimeToNow(comment.updated_at)}
            </div>
        </div>
